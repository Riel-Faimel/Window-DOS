...
void DOScall_handler_c() {
    ...
    auto syscall_number = (u8)((frame->eax >> 8) & 0xFF); //AH
    ...
    case 0x08:  // 无回显键盘输入（处理Ctrl+C）
        ...
        break;
    case 0x09:  // 字符串输出（$结尾）
        print_string(reinterpret_cast<char *>(frame->edx));
        break;
    case 0x0A:  // 缓冲字符串输入
...
DOScall::DOScall(IDT& idt_):idt{idt_} {
    idt_.regist(DOScall_handler, 0x21);
}